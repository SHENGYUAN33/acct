...
> **專案類型**: 企業內部報帳系統
> **版本**: v1.3 (Sprint 3 — Auto Split 自動切割)
> **最後更新**: 2026-04-17
...
- ✅ Auto Split 自動切割（Sprint 3）：連傳多張照片 → N 秒無新圖片 → 自動依憑證斷點切割多筆 Expense
- ✅ trigger_by 標記（`manual_button` / `auto_split`）：Dashboard 顯示觸發來源
...
```
# 認證
POST   /api/v1/auth/login              # 帳號密碼登入，回傳 JWT（含 display_name, employee_id）
POST   /api/v1/auth/register           # 建立新管理員帳號（正式環境應關閉）

# 報帳管理（需 JWT）
GET    /api/v1/expenses                # 清單查詢（含分頁/狀態/日期篩選）
GET    /api/v1/expenses/export         # 匯出 CSV（UTF-8 BOM，最多 10,000 筆）
...
# 圖片管理（需 JWT）
POST   /api/v1/expenses/{id}/images           # 追加圖片（補件）
POST   /api/v1/expenses/{id}/images/replace   # 替換指定索引圖片

# 後台管理（需 JWT）
POST   /api/v1/admin/setup-rich-menu   # 重建 LINE Rich Menu

# 系統
POST   /webhook                        # LINE Webhook（無需 JWT）
GET    /health                         # 健康檢查
```
...
| 表名 | 用途 | 關鍵欄位 |
|------|------|---------|
| `users` | 使用者主表 | `line_user_id` (unique), `department`, `real_name`, `employee_id` |
| `expenses` | 報帳主表（每次批次送出一筆） | `status`, `total_amount`, `serial_number`, `voucher_categories`, `trigger_by` |
| `expense_images` | 批次報帳的各張子圖（Sprint 2 新增） | `expense_id` (FK), `is_voucher`, `voucher_category`, `voucher_subtype`, `expense_category`, `ocr_confidence` |
| `user_states` | LINE 對話狀態（COLLECTING 模式） | `pending_images`, `pending_description`, `step` |
...
| `voucher_categories` | TEXT \| null | 憑證頂層類別去重清單（JSON 陣列字串，如 `["INVOICE","RECEIPT"]`） |

**expenses 表 — Sprint 3 新增欄位：**
...
| `image_url` | VARCHAR(512) | 圖片路徑（uploads/{uuid}.jpg） |
...
- `SUPPLEMENTED` - 已補件（補件 OCR 成功後自動設定，Sprint 2 新增）
