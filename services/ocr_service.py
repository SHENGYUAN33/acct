...
從以下清單選一，填入**完整中文名稱**：
  ...
  **禁止依據（不得作為 expense_category 判斷線索）：**
  - 買受人／抬頭公司名稱或其產業別（例如買受人是電影公司，不代表這筆消費是拍攝相關費用；
    買受人只說明「誰在報帳」，不能說明「花在什麼項目」）
  - 品名欄位手寫字跡潦草、無法確定辨識結果時，不得用猜測的字義去聯想科目
    （例如把模糊字猜成「錄」就聯想成攝影／錄影相關費用）；此時應直接填「勞-未設定」，
    即使因此讓 ocr_confidence 降低也必須如實反映不確定性，不得給高信心分數
...
場景推理範例：
  ...
  - 「品名欄手寫字跡無法確定辨識（如疑似『昭錄』但無把握），即使買受人是電影／製作公司
    → 不得因買受人產業別聯想科目 → expense_category=勞-未設定」
